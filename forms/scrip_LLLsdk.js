...
document.getElementById('dataForm').addEventListener('submit', async function (event) {
    ...
    // document.getElementById('fecha_hora').value = fechaHoraActual;    checar primero si otro modal no
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            const existingMessage = `
                El registro ya existe revisar informacion antes de continuar . Aquí están los datos existentes:
            ...
            if (userConfirmed) {
                ...
                if (foto1) {
                    ...
                    await uploadBytes(foto1Ref, foto1);
                    const foto1URL = await getDownloadURL(foto1Ref);
                    await setDoc(docRef, { foto1URL }, { merge: true });
                }
                ...
                if (foto2) {
                    ...
                    await uploadBytes(foto2Ref, foto2);
                    const foto2URL = await getDownloadURL(foto2Ref);
                    await setDoc(docRef, { foto2URL }, { merge: true });
                }
                ...
                if (foto3) {
                    ...
                    await uploadBytes(foto3Ref, foto3);
                    const foto3URL = await getDownloadURL(foto3Ref);
                    await setDoc(docRef, { foto3URL }, { merge: true });
                }
                ...
                if (foto4) {
                    ...
                    await uploadBytes(foto4Ref, foto4);
                    const foto4URL = await getDownloadURL(foto4Ref);
                    await setDoc(docRef, { foto4URL }, { merge: true });
                }
                ...
                if (foto5) {
                    ...
                    await uploadBytes(foto5Ref, foto5);
                    const foto5URL = await getDownloadURL(foto5Ref);
                    await setDoc(docRef, { foto5URL }, { merge: true });
                }

                if (archivo) {
                    const archivoRef = ref(storage, `documentos/otros/${archivo.name}`);
                    await uploadBytes(archivoRef, archivo);
                    const archivoURL = await getDownloadURL(archivoRef);
                    await setDoc(docRef, { archivoURL }, { merge: true });
                }
                ...
                document.getElementById('numero_telefonico').focus();
                alert('Operación cancelada. Modifique el número de teléfono si es necesario.');
            }
        } else {
            // Documento no existe, proceder a subir archivos y crear nuevo documento
        ...
        console.error('Error al enviar o actualizar el formulario:', error);
